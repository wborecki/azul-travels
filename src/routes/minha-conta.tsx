...
function MinhaContaLayout() {
  ...
  if (loading || !user) return <div className="container mx-auto p-12 text-center text-muted-foreground">Carregando...</div>;
    ...
    <div className="container mx-auto px-4 py-8 grid lg:grid-cols-[240px_1fr] gap-8">
      <aside>
        <div className="bg-card border rounded-2xl p-2 sticky top-20">
          <NavItem to="/minha-conta" icon={<LayoutDashboard className="h-4 w-4" />} label="Dashboard" exact />
          <NavItem to="/minha-conta/perfil-sensorial" icon={<Users className="h-4 w-4" />} label="Perfis sensoriais" />
          <NavItem to="/minha-conta/reservas" icon={<CalendarCheck className="h-4 w-4" />} label="Minhas reservas" />
          <NavItem to="/minha-conta/dados" icon={<UserIcon className="h-4 w-4" />} label="Dados da conta" />
        </div>
      </aside>
