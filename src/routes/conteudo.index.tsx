  ...
  useEffect(() => {
    void (async () => {
      ...
      if (cat !== "todas")
        q = q.eq(
          "categoria",
          cat as "legislacao" | "dicas_viagem" | "boas_praticas" | "novidades" | "destinos",
        );
      const { data } = await q;
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {artigos.map((a) => (
            <Link
              ...
              to="/conteudo/$slug"
              params={{ slug: a.slug }}
              className="group bg-card rounded-2xl overflow-hidden shadow-soft hover:shadow-elegant transition border animate-fade-up"
            >
